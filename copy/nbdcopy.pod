...
=head2 nbdcopy -- [ qemu-nbd -f qcow2 disk.qcow ] disk.raw

Run L<qemu-nbd(8)> as a subprocess to open F<disk.qcow2>, which is
then copied and converted to F<disk.raw>.  Note the use of
S<C<[ ... ]>> to run an NBD server as a subprocess and C<--> to
prevent the qemu-nbd flags from being interpreted as nbdcopy flags.
...
Instead of connecting to an already running server using an C<nbd://>
URI, you can run an NBD server as a subprocess using:
...
subprocesses.  nbdcopy cleans up the server on exit.
...
=head1 COMPARING NBD CONTENT

You can use nbdcopy, L<cmp(1)> and L<bash(1)> process substitution to
compare the content of two NBD servers for equality:

 cmp <( nbdcopy nbd://server1 - ) <( nbdcopy nbd://server2 - )

Note this tests that the content is logically equal.  It does not
compare the NBD metadata such as sparseness (see L<nbdinfo(1)>
I<--map> option).
