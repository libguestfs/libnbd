...
has completed:
 ...
 while (!nbd_aio_command_completed (nbd, id)) {
   if (nbd_poll (nbd, -1) == -1) {
     fprintf (stderr, "%s\n", nbd_get_error ());
     exit (EXIT_FAILURE);
   }
...
do not overlap with other write-like commands in flight — a parallel
read may see indeterminate data, and a parallel write may even cause
disk corruption where the resulting disk contents do not match either
of the two writes.
...
this feature.  Without error handling it would look like this:
 ...
 nbd[0] = nbd_create ();
 nbd[0]_connect_tcp (nbd[0], "server", "10809");
