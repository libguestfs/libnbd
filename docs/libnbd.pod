...
Each manual page covers one function from the C API in detail.
...
Even when a call returns an error, L<nbd_get_errno(3)> might return C<0>.
...
at compile time.  The L<nbd_connect_uri(3)> and L<nbd_aio_connect_uri(3)>
...
(and several more, see C<include/libnbd.h> for all of the C<nbd_is_*>
and C<nbd_can_*> functions available).
...
L<nbd_set_debug_callback(3)>, L<nbd_aio_pread(3)>).  Libnbd can call these
...
For example L<nbd_set_debug_callback(3)> sets up a callback which you
...
(there is no need to call L<nbd_aio_command_completed(3)>); for any other
