...
of these functions will run to completion before returning.  A more
complicated low level non-blocking asynchronous API is also available
where you can integrate with L<poll(2)> or another main loop.
...
example the asynchronous variant of C<nbd_pread> is:
...
=head2 Simple implementation with C<nbd_poll>

In fact if you want to use L<poll(2)> on a single handle, a simple
implementation has already been written called C<nbd_poll>.  It is
...
Some examples of using C<nbd_poll> follow.
...
is a low level asynchronous equivalent (eg. C<nbd_aio_pread>) for
starting a command.
...
at compile time.  The C<nbd_connect_uri> and C<nbd_aio_connect_uri>
calls will raise an error (with errno = C<ENOTSUP>) if it was not
built with this feature, and you can also test for it explicitly using
C<nbd_supports_uri>.
...
If C<nbd_can_trim> returns true, C<nbd_trim> can be used to “punch
...
C<nbd_cache> call (available if C<nbd_can_cache> returns true) is used
to prefetch.
...
like C<nbd_aio_pread>, C<nbd_aio_pwrite>) without waiting for previous
...
To do this you should open a single connection first and test it for
this feature.  Without error handling it would look like this:
...
C<nbd_set_tls> before connecting:
  ...
  * Avoid this mode if man-in-the-middle attacks are a concern.
  */
