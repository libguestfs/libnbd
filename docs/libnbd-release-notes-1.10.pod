...
libnbd 1.10.0 was released on B<XXX September 2021>.
...
L<nbdcopy(1)> now uses a default request size of 2M (instead of 32M).
...
When running the tests, suppress C<$DEBUGINFOD_URLS> because of
performance issues (thanks Mark Wielaard).
...
L<nbd_connect(3)> and L<nbd_aio_connect(3)> documentation has been
